...
export function BottomNav() {
  ...
  // showing through or competing for space.
      ...
      {MOBILE_NAV_ITEMS.map(({ labelKey, path, icon: Icon }) => {
        ...
        return (
          <NavLink
            ...
            onClick={(e) => {
              // Tapping Caddie while an analysis just finished jumps
              // straight to that result (same convenience the old banner
              // gave) instead of the plain hub page — viewing it is what
              // clears the dot.
            ...
            {({ isActive }) => (
              <>
                ...
                <span className="relative flex h-7 items-center justify-center">
                  {isCaddie ? (
                    <CaddieNavStatusIcon size={28} strokeWidth={isActive ? 2.5 : 2} />
                  ) : (
