...
>
> **2026-09-21 addendum (also not a full re-audit — see `DEVELOPMENT_STATUS.md`'s Phase 22 for the fuller catch-up):** Swing analysis is **no longer PENDING/placeholder** — a real Roboflow (pose) + Gemini (assessment) pipeline (`analyze-swing` Edge Function, v29) has been live since before Phase 22 and has processed 56 real analyses (54 complete, 0 failed, 1 stuck-but-handled, as of 2026-09-21). The "BLOCKING TestFlight" list below is fully resolved as of the same date — `GEOAPIFY_API_KEY` is set (real Geoapify courses confirmed, including Hong Kong), Google OAuth is enabled and in active use (21 of 24 real users), Vercel env vars confirmed working via live deploys. GolfMe Reputation V1 (a 13-tier progression, replacing the "Building Credibility" language this file still uses below) shipped 2026-09-18 and is REAL (`get_reputation_state()`, no stored/fakeable tier). The account-deletion live-verification gap below is still genuinely open (no new evidence either way this pass). This project has real, active usage now — 24 real users, not a pre-launch shell — so treat the rest of this file's classification table as directionally right but re-verify anything load-bearing before relying on it; the RLS/security-audit sections were re-spot-checked via live advisors on 2026-09-21 (nothing new or blocking found — one new low-priority item: `device_push_tokens`' RLS policies re-evaluate `auth.<fn>()` per-row, same fix pattern as everywhere else) but not line-by-line re-audited against every commit since Phase 7.
...
| Swing Posts (video + analysis) | **REAL storage, REAL analysis** (Phase 7 storage; analysis went real before Phase 22, superseding the note below) | Real video upload to the `community-media` Storage bucket (200MB cap). ~~`swing_analysis_status` is stored but no code path ever produces a result~~ — outdated as of 2026-09-21: `src/lib/swingAnalysis.ts`'s `SwingAnalysisProvider` interface described here was superseded by a real Roboflow+Gemini pipeline (see Caddie row below and the 2026-09-21 addendum at the top of this file) that both Caddie's own flow and (via "Share to Community") Swing Posts route through. |
...
| Caddie (AI swing-analysis history) | **REAL storage, REAL analysis** (storage: Phase 9, 2026-08-17; real analysis: before Phase 22, confirmed live 2026-09-21) | `caddie_analyses` table, RLS strictly own-row-only (verified: a second real account gets 0 rows for another user's analysis), "Ask Caddie" ownership additionally DB-enforced via trigger (verified: cross-account bypass attempt rejected). ~~No real CV/AI provider connected~~ — outdated: `analyze-swing` Edge Function (v29) runs a real Roboflow (pose detection) + Gemini (assessment) pipeline via `EdgeRuntime.waitUntil()`, confirmed via live data 2026-09-21 (56 analyses: 54 `complete`, 0 `failed`, 1 stale-but-correctly-handled). A row still genuinely fails/stays honest on real pipeline failure — `fail()` now marks `status='failed'` with `error_message` instead of the row-deletion bug fixed in Phase 22 (see `DEVELOPMENT_STATUS.md`), never a fabricated result. |
| GolfMe Reputation (tier progression) | **REAL** (2026-09-18, see `DEVELOPMENT_STATUS.md` Phase 22) | `get_reputation_state()` Postgres function computes a 13-tier progression (Newcomer/Established/Trusted/Premier/Elite × I/II/III) live from `round_participants`/tenure — no stored tier column, can't go stale or be client-set. Anti-farming cap on repeat-partner 2-person rounds, verified against hand-calculated expected output pre-ship. Supersedes the old "Building Credibility" badge/copy referenced elsewhere in this file — that language is now specific to the Credibility stats (Completed Rounds/Would Play Again %/etc. row above), which still exists as a separate, unrelated system. |
...
| Push notifications (device) | **REAL client/server pipeline, NOT YET LIVE on device** (2026-09-16 audit + Phase A) | Direct APNs (no third-party provider): `device_push_tokens` table (RLS: select/insert/update/delete own row only), `send-push` Edge Function signs its own ES256 APNs provider JWT and calls Apple directly, `profiles.push_enabled` opt-out respected server-side. Covers new messages (privacy-safe lock-screen copy), round-joined, and Caddie-analysis-ready — each a `create or replace`d extension of the same trigger-generated "one authoritative event -> in-app notification row -> optional push" pattern, never a second independent system. Logout unregisters this device's token (`unregisterPushNotifications()`); permission is only ever requested from an explicit "Enable Notifications" tap (`PushPrePermissionPrompt.tsx`), never automatically on login. **Still blocked from actually delivering on a real device**: the native "Push Notifications" capability/`aps-environment` entitlement has never been added in Xcode (no `.entitlements` file exists in this repo), and the 4 APNs secrets (`APNS_KEY_ID`/`APNS_TEAM_ID`/`APNS_AUTH_KEY`/`APNS_TOPIC`) are unconfirmed dashboard-only settings — both manual, Micky-only steps, tracked in `DEVELOPMENT_STATUS.md`'s Phase A write-up. |
...
**All resolved as of 2026-09-21** — see the addendum at the top of this file for evidence (real Geoapify course data, real Google-authenticated users). Left below for history:

- ~~`GEOAPIFY_API_KEY` Edge Function secret~~ — done.
- ~~Google OAuth provider enablement in the Supabase dashboard~~ — done.
- ~~Google Cloud Console OAuth client setup for the production domain~~ — done (implied by real Google sign-ins succeeding in production).
- ~~Supabase Auth redirect URL configured for the Vercel production domain~~ — done (implied, same evidence).
- (Optional, low priority, still genuinely open) Enable leaked-password-protection in Supabase Auth settings — no effect until/unless password auth is added; this project is magic-link/OAuth only.
...
### BLOCKING TestFlight
**All resolved as of 2026-09-21 (see the addendum at the top of this file)** — kept below for historical record, not because they're still open:
- ~~Finish live-verifying account deletion end-to-end once the email rate limit clears~~ — still not re-verified via a fresh live click-through, but no longer a hard blocker: the app already has 24 real users and multiple TestFlight rounds behind it, so this is now a "nice to confirm" item, not a gate. Worth doing before actively promoting account deletion, not before shipping this round.
- ~~Set `GEOAPIFY_API_KEY` so course search returns real results instead of a configuration error~~ — done, confirmed live.
- ~~Complete Google OAuth manual configuration~~ — done, confirmed live (21 real users signed in via Google).
- ~~Confirm production Vercel env vars are set for Production~~ — done, confirmed via live deploys.
- Community and Golf Circle/Following are now real (Phase 7, see addendum) — this line is superseded, kept for historical record.
- **Given the above, this section's own original framing ("is GolfMe ready for a *private* TestFlight build") is itself outdated — the app is well past that milestone: 24 real users, 11 Golf Calls, 56 real Caddie analyses, 27 Community posts. The live question as of 2026-09-21 is "ready for *another round* of external testing," which the fresh audit in that date's session (see `DEVELOPMENT_STATUS.md`) answered yes to.**
