...
| Caddie (AI swing-analysis history) | **REAL storage, PENDING analysis** (Phase 9, 2026-08-17) | `caddie_analyses` table, RLS strictly own-row-only (verified: a second real account gets 0 rows for another user's analysis), "Ask Caddie" ownership additionally DB-enforced via trigger (verified: cross-account bypass attempt rejected). No real CV/AI provider connected — `status` stays `'pending'` forever, same honest-placeholder posture as Swing Posts, never a fabricated result. |
